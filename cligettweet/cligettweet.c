 ...
 * Compilacion:    make
 * Uso:            cligettweet <direccionIP> <puerto> <tambuffer> <topico>
 * Funcion:        cliente de recepcion de tweets.
 *                 Realiza conexiones a un servidor <direccionIP> por el puerto <puerto>
 *		   Envia el <topico> por la conexion TCP y llama a la funcion int gettweet
 *		   Informa al usuario si la funcion devuelve -1 y cierra la conexion.
...
main(int argc, char **argv)	{
	...
	if (argc != 5)	{
		fprintf(stderr,"Uso: cligettweet <direccionIP> <puerto> <buffer> <topico>\n");
		exit(1);
	}
	...
	/*Llamamos a la funcion gettweet para recibir el mensaje e informamos al usuario del fracaso, si lo hubiera*/
	if((salida = gettweet(sockfd, buffer, tambuffer)) != 0) {
		fprintf(stderr, "Error en la recepcion del tweet :(\n");
		close(sockfd);
		exit(1);
	}
